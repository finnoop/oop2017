...
public class Account {
    ...
    protected void addRule(BookingRule rule) {
        rules.add(rule);
    }

    private BigDecimal calcValue(BigDecimal value) {
        BigDecimal balance = value;
        rules.stream().reduce(balance, (acc, r) -> r.calculateValue(acc), BigDecimal::add);
        return balance;
    }
